...
def get_sizes_for_family(family):
    ...
    try:
        ...
        sql = """
          ...
          ORDER BY name;
        """
        df = pd.read_sql(sql, conn, params=(family,))
...
def get_section_row(family, name):
    if not HAS_PG:
        df = pd.DataFrame(SAMPLE_ROWS)
        df_f = df[(df['family'] == family) & (df['name'].astype(str) == str(name))]
        if df_f.empty:
            return None
...
# -------------------------
# UI: family -> size -> load properties
# -------------------------
...
if family and family != "-- choose --":
    ...
    if not sizes:
        st.info("No section sizes found for chosen family.")
